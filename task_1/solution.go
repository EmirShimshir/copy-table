...
func CopyTable(connPool ConnectionPool, fromName string, toName string, full bool) error {
	...
	for offset := startID + 1; offset <= maxID; offset += batchSize {
		rows, err := callWithRetry(ctx, func(ctx context.Context) ([]Row, error) {
			return fromDB.LoadRows(ctx, offset, offset+batchSize)
		})
		if err != nil {
		...
		if _, err := callWithRetry(ctx, func(ctx context.Context) (struct{}, error) {
			err = toDB.SaveRows(ctx, rows)
			return struct{}{}, err
		}); err != nil {
			return fmt.Errorf("save rows [%d,%d): %w", offset, offset+batchSize, err)
		}
	}
